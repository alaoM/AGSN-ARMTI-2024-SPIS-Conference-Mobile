...
const ConferenceMaterials = ({ navigation }) => {
  ...
  const handleFileAction = async (fileUrl, fileName, action) => {
    try {
      ...
      const fullFileUrl = `${API_URL}/files/download/${fileName}`;
  ...
  const renderMaterialItem = ({ item }) => {
    ...
    const renderFilePreview = () => {
      if (['jpg', 'jpeg', 'png', 'gif'].includes(fileExtension)) {
        return <Image source={{ uri: `${API_URL}${item.url}` }} style={styles.previewImage} contentFit='contain'
